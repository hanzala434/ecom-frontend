...
const RecentOrderCard = ({order}) => {
    ...
    const handleClick=()=>{
        navigate(`/your-orders/order/${order._id}`)
    }
  return (
   <>
...
<div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-4">
    ...
    <div class="p-5">
        <a onClick={handleClick}>
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{order.cartItems[0].name}</h5>
        </a>
        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{order.orderDate}</p>
